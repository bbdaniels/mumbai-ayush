...
// Tabulating SP cases in qutub_sample_updated in wave 0 and 1
	...
	forvalues i = 8/11 {
		tab2xl case if qutub_sample_updated == `i' & wave == 0 ///
			using "${directory}/outputs/sp_case_tabs.xlsx" , col(1) row(`row') //Export tables for Wave0
		tab2xl case if qutub_sample_updated == `i' & wave == 1 ///
			using "${directory}/outputs/sp_case_tabs.xlsx" , col(6) row(`row') // Export tables for Wave1
...
// Creating Balance Tables and Graphs
  ...
  // Transform Analysis Variables
	...
	unab Balance : cp_17_* cp_18 cp_19 cp_20 cp_21 ///
				   ...
				   correct* cb* dr_1 dr_4 re_1 re_3 re_4 med_any med med_l_any_1 ///
	 ...
	 // Create balance table graphs 
		...
		xlab(, labsize(vsmall)) graphregion(color(white) lwidth(large))) 
			...
			graph export "${directory}/outputs/`i'.eps", replace 
